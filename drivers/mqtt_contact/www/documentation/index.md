...
The MQTT Contact driver provides monitoring of MQTT-based contact sensors
through a Control4 contact sensor proxy. It subscribes to a state topic to
receive open/closed updates and can monitor device availability.
...
- Monitor MQTT contact sensors via Control4 contact sensor proxy
- Subscribe to state topics for open/closed updates
- Monitor device availability via availability topic
- Configurable state payloads and QoS settings
...
2. Add the "MQTT Contact" driver to your project.
3. In the Connections tab, bind the MQTT Contact to the MQTT Broker.
...
The MQTT topic to subscribe for contact state updates. This is required.

##### State Open

The payload that indicates the contact is open. Default is `OPEN`.

##### State Closed

The payload that indicates the contact is closed. Default is `CLOSED`.
...
MQTT Quality of Service level for subscriptions. Default is `0`.
